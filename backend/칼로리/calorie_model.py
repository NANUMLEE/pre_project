# calorie_predictor.py
import numpy as np
import joblib
from catboost import CatBoostRegressor


# ====================================
# 1. 전처리 함수 (BMI 포함)
# ====================================
def preprocess_user_input(age, height, weight, duration):
    """
    사용자 입력 데이터를 모델 입력 형태로 변환하는 함수
    Feature 순서: ["duration", "weight", "height", "bmi", "age"]
    """

    age = float(age)
    height = float(height)
    weight = float(weight)
    duration = float(duration)

    # BMI 계산
    bmi = weight / ((height / 100) ** 2)

    return [duration, weight, height, bmi, age]


# ====================================
# 2. 모델 로딩 (cbm → pkl 순서로 로딩)
# ====================================
def load_model():
    """
    저장된 CatBoost 모델을 불러오는 함수
    우선순위: .cbm → .pkl
    """
    model = CatBoostRegressor()

    try:
        model.load_model("catboost_calorie_model.cbm")
        print("✔ CatBoost .cbm 모델 로드 완료")
        return model
    except Exception:
        pass

    try:
        model = joblib.load("catboost_calorie_model.pkl")
        print("✔ CatBoost .pkl 모델 로드 완료")
        return model
    except Exception:
        raise FileNotFoundError(
            "❌ catboost_calorie_model.cbm 또는 .pkl 파일을 찾을 수 없습니다."
        )


# 모델 1회 로딩 (FastAPI에서도 효율적)
model = load_model()


# ====================================
# 3. 예측 함수
# ====================================
def predict_calorie(age, height, weight, duration):
    """
    입력값을 받아 칼로리를 예측하는 함수
    """
    X = np.array([
        preprocess_user_input(age, height, weight, duration)
    ])

    pred = model.predict(X)

    return float(pred[0])

#catboost_calorie_model.cbm → CatBoost native 모델 (속도 빠르고 안정적)
#catboost_calorie_model.pkl → joblib 기반 모델 (범용 Python 호환)