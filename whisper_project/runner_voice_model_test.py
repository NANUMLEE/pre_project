...
Runner = Dict[str, str]  # {"id": "1", "name": "홍길동"}


# =========================
# 1. Whisper STT + 명령어 분류 (메모리 기반)
# =========================

# Whisper 모델은 서버 시작 시 1번만 로드
stt_model = whisper.load_model("small")


def extract_emoji_type_from_text(text_no_space: str) -> Optional[str]:
    """
    STT 텍스트(공백 제거본)에서 이모티콘 종류 추출
    :return: "FIGHTING" / "HIGHFIVE" / "FIRE" 또는 None
    """
...
def decode_audio_bytes_to_waveform(audio_bytes: bytes) -> np.ndarray:
    """
    디스크에 저장하지 않고 메모리 상의 음성 바이트를
    ffmpeg로 16kHz mono PCM waveform(float32)으로 변환.
    """
    cmd = [
        "ffmpeg",
        "-i", "pipe:0",       # stdin에서 입력
    ...
    if proc.returncode != 0:
        ...
        raise RuntimeError(f"ffmpeg 변환 실패: {err}")
...
def process_audio_bytes(audio_bytes: bytes) -> dict:
    """
    음성 바이트(파일 저장 X)를 받아서:
    1) ffmpeg로 16kHz waveform 변환
    2) Whisper STT
    3) 명령어 분류
    """
    waveform = decode_audio_bytes_to_waveform(audio_bytes)
    w = stt_model.transcribe(waveform, language="ko")
    text = w["text"].strip()
...
def send_emoji_to_runners(runner_ids: List[str],
    ...
    """
    실제 이모티콘 전송 자리.
    지금은 print만 하고, 나중에 WebSocket 브로드캐스트로 교체.
    """
...
def map_emoji_str_to_enum(emoji_str: Optional[str]) -> EmojiType:
    """
    "FIGHTING"/"HIGHFIVE"/"FIRE" 문자열을 EmojiType으로 변환.
    """
...
async def text_to_speech(text: str, out_path: str = "response.mp3") -> io.BytesIO:
    """
    입력 텍스트를 edge-tts로 mp3로 변환.
    - mp3 BytesIO를 리턴
    - out_path 경로에 파일도 저장(디버그용)
    """
...
async def run_pipeline_bytes(audio_bytes: bytes,
    ...
    """
    1) 메모리의 음성 바이트로 STT + 명령어 분류
    2) 명령어 & 주변 러너 기반 응답 문장 생성
    3) Edge TTS로 음성(mp3) 생성
    """
    command_info = process_audio_bytes(audio_bytes)
...
# =========================
# 5. (선택) 로컬 테스트용 main
#    - 파일을 한번 읽어서 bytes로 만든 뒤 run_pipeline_bytes 호출
# =========================

if __name__ == "__main__":
    # ⚠ 여기 부분은 "파일 → bytes로 읽어서 테스트"만 하는 용도야.
    # 실제 서비스에서는 FastAPI에서 UploadFile.read()로 받은 bytes를 그대로 넣으면 됨.
    test_audio_path = r"C:\Users\sinhy\OneDrive\바탕 화면\project\whisper_project\voice\녹음.m4a"
