...
export function Community({ onNavigate }: CommunityProps) {
  ...
  const avgPace = records.reduce((sum, r) => sum + r.pace, 0) / records.length;
  const avgDistance = totalDistance / records.length;
  const maxDistance = Math.max(...records.map(r => r.distance));
            ...
            {records.length > 2 && (
              <div className="pt-2 border-t border-gray-200">
                <p className="text-xs text-center text-[#787878]">
                  아래로 스크롤하여 {records.length - 2}개의 기록을 더 볼 수 있습니다
                </p>
